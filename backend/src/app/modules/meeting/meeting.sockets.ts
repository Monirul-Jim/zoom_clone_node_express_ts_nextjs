...
export const handleSocketConnection = (io: Server) => {
  io.on('connection', (socket: Socket) => {
    ...
    //     console.log(meeting,'from join meeting')
    ...
    socket.on('join-meeting', async ({ meetingId, token }) => {
      try {
        ...
        const participants = await RegistrationModel.find({
          _id: { $in: meeting.participants }
        }).select('_id firstName lastName');

        // 🔥 Send participants list to the newly joined user
  ...
  try {
    ...
    const userId = decoded._id;
    console.log(token,'jdjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj')
